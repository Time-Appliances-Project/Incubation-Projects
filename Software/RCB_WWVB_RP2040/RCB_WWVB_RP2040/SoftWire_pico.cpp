...
{
  result_t r;
  //Serial.println("Softwire endTransmissionInner start");
  ...
  if (r == nack) {
    //Serial.println("Softwire endTransmissionInner nack");
    return 2;
  }
  else if (r == timedOut) {
    //Serial.println("Softwire endTransmissionInner timedOUt");
    return 4;
  }
  //Serial.println("Softwire endTransmissionInner start write");

  for (uint8_t i = 0; i < _txBufferIndex; ++i) {
    r = llWrite(_txBuffer[i]);
    if (r == nack) {
      //Serial.println("Softwire endTransmissionInner write nack");
      return 3;
    } else if (r == timedOut) {
      //Serial.println("Softwire endTransmissionInner write timed out");
      return 4;
    } else {
      //Serial.println("Softwire endTransmissionInner write success");
    }
  }
