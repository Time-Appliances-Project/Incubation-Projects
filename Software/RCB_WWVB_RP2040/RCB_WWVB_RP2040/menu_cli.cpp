...
void init_cli()
{
  if ( started_cli_before ) 
  {
    ...
    vTaskDelete(cliThread); //  make sure this thread is dead 
  }
  ...
  */
  ...
  xTaskCreate( cli_loop, "cli_loop", 4096, NULL, tskIDLE_PRIORITY, &cliThread);
...
static Node i2c_read_node = { .name = "", 
  .type = MY_FILE, 
  .cliBinding = {"read",
    "I2C Read",
    true,
...
static Node i2c_write_node = { .name = "", 
  .type = MY_FILE, 
  .cliBinding = {"write",
    "I2C write",
    true,
...
{
  Serial.println("Not sure how to reboot RP2040 from code!");
  //Serial.println("User command, restarting RP2040!\r\n");
  //delay(200);
  //watchdog_reboot();
  //machine.reset();
  //NVIC_SystemReset();
